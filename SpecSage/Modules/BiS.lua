...
-- Ring and Trinket each cover two inventory slots; Weapon and Off-hand map
-- to the main-hand/off-hand slots. Everything else is a single slot. These
-- are Blizzard's fixed INVSLOT_* inventory slot IDs, listed as plain numbers
-- since the addon has no other need for the named constants.
...
-- the client has not cached yet (a fresh itemID lookup can be async), so
-- "no name yet" is a normal outcome here, not an error.
